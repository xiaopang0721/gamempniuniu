...
* name 
*/
module gamempniuniu.data {
	export class MpniuniuMapInfo extends gamecomponent.object.MapInfoT<MpniuniuData> {
		//地图状态变更
		...
		constructor(v: SceneObjectMgr) {
			super(v, () => { return new MpniuniuData() });
		...
		protected onUpdate(flags: number, mask: UpdateMask, strmask: UpdateMask): void {
			...
			if (isNew || mask.GetBit(MapField.MAP_INT_MAP_BYTE)) {
				this._sceneObjectMgr.event(MpniuniuMapInfo.EVENT_STATUS_CHECK);
			}
			if (isNew || mask.GetBit(MapField.MAP_INT_BATTLE_INDEX)) {
				this._battleInfoMgr.OnUpdate();
				this._sceneObjectMgr.event(MpniuniuMapInfo.EVENT_BATTLE_CHECK);
			}
			if (isNew || mask.GetBit(MapField.MAP_INT_MAP_BYTE1)) {
				this._sceneObjectMgr.event(MpniuniuMapInfo.EVENT_GAME_ROUND_CHANGE);
			}
			if (isNew || mask.GetBit(MapField.MAP_INT_COUNT_DOWN)) {
				this._sceneObjectMgr.event(MpniuniuMapInfo.EVENT_COUNT_DOWN);
			}
			if (isNew || strmask.GetBit(MapField.MAP_STR_GAME_NO)) {
				this._sceneObjectMgr.event(MpniuniuMapInfo.EVENT_GAME_NO);
			}
		}
		//牌型
		...
		public getBattleInfoToString(): string {
			...
			for (let i = 0; i < this._battleInfoMgr.info.length; i++) {
				...
				if (battleInfo.Type == 12) {
					...
					let newString = name + "：" + "摊牌，牌型是：" + MpniuniuMapInfo.cardType[info.CardType - 1];
					str = str + "#" + newString;
